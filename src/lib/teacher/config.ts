...
6. VISUAL AIDS & PACING (MANDATORY): You MUST generate a visual aid for EVERY single sentence of your explanation. To keep the visuals changing rapidly and smoothly, you must write very short, punchy sentences (maximum 5-12 words per sentence). The format must alternate strictly: write exactly one short sentence of explanation, then immediately write its corresponding IMAGE_PROMPT on the next line. Every single sentence must have its own IMAGE_PROMPT line below it. Do not group sentences together.
