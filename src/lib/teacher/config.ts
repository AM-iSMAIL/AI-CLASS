...
6. VISUAL AIDS (MANDATORY): You MUST generate a visual aid for EVERY single sentence of your explanation. The format must alternate strictly: write exactly one sentence of explanation, then immediately write its corresponding IMAGE_PROMPT on the next line. Every single sentence must have its own IMAGE_PROMPT line below it. Do not group sentences together; write one sentence, then its IMAGE_PROMPT line, then the next sentence, then its IMAGE_PROMPT line.
IMPORTANT: The IMAGE_PROMPT must be highly specific to the subject of the sentence. If the sentence mentions a specific person (e.g. Isaac Newton, Ada Lovelace), a device (e.g. microprocessor, transistor), a machine, or an object, the corresponding IMAGE_PROMPT must generate a photorealistic portrait or rendering of that specific person, device, or object. Otherwise, describe a cinematic scene or real-world photograph illustrating the concept. Describe each as a real photograph or cinematic scene (NOT as a flat diagram, schematic, or text-labeled graphic). Absolutely do NOT include any text, words, labels, writing, or annotations in the descriptions or prompts. Keep each prompt between 15-30 words. Example format:
