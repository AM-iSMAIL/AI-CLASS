...
export default function SessionPage() {
  ...
  const ttsQueueRef = useRef<Array<{ text: string; onEnd?: () => void; audio?: HTMLAudioElement | null; promise?: Promise<any> | null; error?: boolean; imagePrompt?: string; imageUrl?: string | null; imagePromise?: Promise<any> | null; runId: number }>>([])
  ...
  const triggerAudioSpeech = (text: string, onEnd?: () => void) => {
    ...
    // Split text into sub-chunks of max 75 characters to prevent Camb AI latency
    const maxLen = 75;
    const subChunks: string[] = [];
    if (filteredText.length <= maxLen) {
      subChunks.push(filteredText);
    } else {
      const sentences = filteredText.match(/[^.!?\n]+[.!?\n]*/g) || [filteredText];
      let currentChunk = "";
      for (const sentence of sentences) {
        if ((currentChunk + sentence).length > maxLen) {
          if (currentChunk.trim()) subChunks.push(currentChunk.trim());
      ...
      // If any sentence is itself longer than 400 chars, split by word count
      ...
      subChunks.length = 0;
      subChunks.push(...finalChunks);
    }

    // Queue all sub-chunks sequentially, and start prefetching their audios in parallel immediately
    subChunks.forEach((chunk, index) => {
      const isLast = index === subChunks.length - 1;
      const item = {
        text: chunk,
  ...
  const runAiTopicSpeech = async (topicIdx: number) => {
    ...
    const prevTopics = session.topics.slice(0, topicIdx)
    classroomContext.updateState({
      topic: topic,
  ...
  const handleSendDoubt = async (e: React.FormEvent, question: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Filter out any IMAGE_PROMPT lines from the displayed text
        const cleanAnswerText = data.answer
          .split("\n")
          .filter((l: string) => !l.trim().startsWith("IMAGE_PROMPT:"))
          .join("\n")
          .trim();
